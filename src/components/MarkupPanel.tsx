...
export default function MarkupPanel({ markup, compact = false, direction = 'up', embedded = false }: MarkupPanelProps) {
  ...
  const ToolBtn = ({ tool, label, icon }: { tool: MarkupTool; label: string; icon: React.ReactNode }) => (
    <button
      ...
      className={`flex items-center gap-1.5 px-2.5 py-1 rounded text-[11px] font-medium transition-colors ${
        state.tool === tool ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white hover:bg-slate-700'
      }`}
    >
      {icon}
      {label}
  ...
  const panelContent = (
    <div className="min-w-[280px] max-h-[min(70vh,600px)] overflow-y-auto bg-slate-800 border border-slate-600 rounded-lg shadow-xl p-3 space-y-3">
      {/* Draw tools */}
      <div>
        <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1.5">Draw</div>
        ...
        {state.tool === 'line' && <p className="text-[10px] text-slate-500 mt-1.5">Click two points to draw</p>}
        {state.tool === 'angle' && <p className="text-[10px] text-slate-500 mt-1.5">Click 3 points: start → vertex → end</p>}
        {state.tool === 'text' && <p className="text-[10px] text-slate-500 mt-1.5">Click to place, type, Enter. Double-click text to edit.</p>}
      </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {state.tool === 'line' && (
          <>
            <span className="text-[10px] text-slate-500 ml-1">Width</span>
            <input type="range" min={1} max={8} step={1} value={state.lineWidth} onChange={(e) => setLineWidth(parseInt(e.target.value))} className="w-20" />
            <span className="text-[10px] text-slate-300 w-6 shrink-0">{state.lineWidth}px</span>
          </>
        )}
        {state.tool === 'text' && (
          <>
            ...
            <span className="text-[10px] text-slate-300 w-8 shrink-0">{state.textSize}px</span>
          </>
        )}
      </div>
      ...
      {state.selected?.type === 'text' && (() => {
        ...
        return (
          <div className="border-t border-slate-700 pt-2 space-y-1">
            <div className="text-[10px] text-slate-500 uppercase">Edit text</div>
            <input
              type="text"
              value={t.content}
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-slate-300 w-8">{t.size}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[10px] text-slate-500">Color</span>
      ...
      <div className="border-t border-slate-700 pt-2.5 space-y-2">
        <div className="flex items-center justify-between">
          <span className="text-[10px] text-slate-500 uppercase tracking-wider">Markups</span>
        ...
        <div className="flex flex-wrap gap-1.5">
          <button onClick={undo} disabled={state.undoStack.length === 0} className="px-2 py-0.5 text-[10px] rounded bg-slate-700 text-slate-300 hover:bg-slate-600 disabled:opacity-40 disabled:cursor-not-allowed" title="Undo">
            Undo
          </button>
          <button onClick={redo} disabled={state.redoStack.length === 0} className="px-2 py-0.5 text-[10px] rounded bg-slate-700 text-slate-300 hover:bg-slate-600 disabled:opacity-40 disabled:cursor-not-allowed" title="Redo">
            Redo
          </button>
          {hasMarkup && (
            <button onClick={clearAll} className="px-2 py-0.5 text-[10px] rounded text-red-400 hover:bg-slate-700" title="Delete all">
              Delete all
            </button>
          )}
        </div>
          ...
          <div className="space-y-1 max-h-32 overflow-y-auto">
            {state.lines.map((l) => (
              <div
                ...
                className={`flex items-center justify-between gap-1 px-2 py-1 rounded text-[10px] cursor-pointer ${state.selected?.type === 'line' && state.selected.id === l.id ? 'bg-blue-600/40 text-white' : 'bg-slate-700/50 text-slate-300 hover:bg-slate-700'}`}
              >
                <span>Line</span>
            ...
            {state.angles.map((a) => (
              <div
                ...
                className={`flex items-center justify-between gap-1 px-2 py-1 rounded text-[10px] cursor-pointer ${state.selected?.type === 'angle' && state.selected.id === a.id ? 'bg-blue-600/40 text-white' : 'bg-slate-700/50 text-slate-300 hover:bg-slate-700'}`}
              >
            ...
            {state.texts.map((t) => (
              <div
                ...
                className={`flex items-center justify-between gap-1 px-2 py-1 rounded text-[10px] cursor-pointer truncate ${state.selected?.type === 'text' && state.selected.id === t.id ? 'bg-blue-600/40 text-white' : 'bg-slate-700/50 text-slate-300 hover:bg-slate-700'}`}
              >
      ...
      <div className="border-t border-slate-700 pt-2.5 space-y-2">
        <div className="flex items-center justify-between">
          <span className="text-[10px] text-slate-500 uppercase tracking-wider">Grid</span>
          <label className="flex items-center gap-1.5 cursor-pointer">
        ...
        {state.grid.show && (
          <div className="space-y-2 pl-0.5">
            <div className="flex items-center gap-1">
              {(['both', 'horizontal', 'vertical'] as const).map((m) => (
                <button
                  ...
                  className={`px-2 py-0.5 text-[10px] rounded capitalize transition-colors ${state.grid.mode === m ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white hover:bg-slate-700'}`}
                >
                  {m}
                </button>
              ))}
            ...
            <div className="flex items-center gap-2">
              <span className="text-[10px] text-slate-500 w-11 shrink-0">Spacing</span>
              ...
              <span className="text-[10px] text-slate-300 w-10 shrink-0 text-right">{state.grid.spacingPx}px</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[10px] text-slate-500 w-8 shrink-0">Origin X</span>
              ...
              <span className="text-[10px] text-slate-300 w-10 shrink-0 text-right">{state.grid.originX}px</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[10px] text-slate-500 w-8 shrink-0">Origin Y</span>
              ...
              <span className="text-[10px] text-slate-300 w-10 shrink-0 text-right">{state.grid.originY}px</span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-slate-500 ml-1">Opacity</span>
              ...
              <span className="text-[10px] text-slate-300 w-8 shrink-0 text-right">{Math.round(state.grid.opacity * 100)}%</span>
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <div ref={containerRef} className="relative inline-block">
      <button
        onClick={() => setOpen((o) => !o)}
        className={`flex items-center gap-1.5 ${labelSize} transition-colors ${isActive ? 'text-blue-400 hover:text-blue-300' : 'text-slate-500 hover:text-slate-300'}`}
      >
        <svg className="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Z" />
        </svg>
        Markup
        {isActive && <span className="text-[9px] bg-blue-500/20 text-blue-400 rounded px-1">active</span>}
